...
- **2026-07-06 (49): 🖼️ VÁ LỖI ẢNH MEDIA CHẾT 404 (do chính bước build ở mục 48 gây ra) + tunnel tự retry.** Sau khi khởi động qua tunnel, user báo "sao ko có hình" — toàn bộ ảnh media library + avatar kênh vỡ. **KHÔNG phải lỗi tunnel** (404 cả localhost lẫn tunnel). Nguyên nhân: mục 48 tôi build bằng `corepack pnpm run build` → frontend chạy `next build` TRẦN (script `build`, KHÔNG nạp .env), trong khi `run.mjs` luôn dùng `build:sentry` (có dotenv). next.config đọc `STORAGE_PROVIDER` để quyết định đích `/uploads` và **Next đóng băng rewrites/redirects vào `.next/routes-manifest.json` lúc BUILD** → thiếu env → mọi `/uploads/*` cố định thành `/404` (route handler `(app)/api/uploads` đọc `UPLOAD_DIRECTORY` vẫn đúng nhưng không bao giờ được gọi). **Sửa GỐC (commit 637f418):** next.config map `/uploads`→`/api/uploads` VÔ ĐIỀU KIỆN (bỏ phụ thuộc `STORAGE_PROVIDER` lúc build) — dự án luôn local; cloud sinh URL công khai không qua đây. Điều này cũng chống bẫy khi deploy Docker/CI (build-time thường thiếu env). Rebuild `build:sentry` + restart frontend → verify: file 3.16MB tải qua tunnel khớp đúng 3.163.445 bytes, `/uploads/*` = HTTP 200 image/png. Xem memory [[media-hub-frontend-mode]]. **Tunnel:** lần start đầu Cloudflare quick-tunnel lỗi tạm thời `invalid UUID length: 0` (retry là lên) → vá `tunnel.mjs` tự retry (commit 5ed6b6e). **BÀI HỌC: build frontend LUÔN qua `build:sentry`/`run.mjs --rebuild`, KHÔNG `pnpm run build` trần; verify media bằng curl /uploads/<file> phải 200.**
