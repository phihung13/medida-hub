/* Service worker "trang bảo trì sư tử" — khi server sập lúc deploy (Traefik trả
   502/503/504 hoặc đứt kết nối), trình duyệt hiện trang mascot thay cho lỗi trần
   "no available server". Trang tự ping mỗi 5s — server sống lại là tự vào app.
   Chỉ chặn ĐIỀU HƯỚNG TRANG (mode navigate); mọi request API đi thẳng, không đụng.
   LƯU Ý: HTML bên dưới có bản SONG SINH ở docs/cloudflare-worker-bao-tri.js
   (tầng Cloudflare cho khách chưa từng mở app) — sửa 1 nơi nhớ sửa cả 2. */
...
const maintenance = () =>
  new Response(MAINTENANCE_HTML, {
    status: 503, // giữ mã 5xx thật cho bot/monitor — trình duyệt vẫn render body
    headers: {
